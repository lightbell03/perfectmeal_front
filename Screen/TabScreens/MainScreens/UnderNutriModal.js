...
const UnderNutriModal = ({open, onClose}) => {
  ...
  const ShowUnderNutriList = () => {
      
    if(state.underNutri == undefined)
        return (
            <Text>먹은 음식이 없어요</Text>
        ) 
    else
      return Object.entries(state.underNutri).map((key, index) => {
        if(index == 0)
          return(
            <View key={index}>
              <View
              ...
              >
                ...
                <View style={{flexDirection: 'row'}}>
                  <Text style={{color: '#ffb119', fontWeight: 'bold', fontSize: 35}}>{state.underNutri[NutrientEngNameList[index]]}</Text>
          ...
          return (
            ...
            >
              <Text style={{color: '#000000', fontWeight: 'bold', fontSize: 20}}>{NutriNameList[index]}</Text>
              <Text style={{color: '#b7b7b7', fontWeight: 'bold', fontSize:20, marginRight: 5}}>{state.underNutri[NutrientEngNameList[index]]}</Text>
            </View>
          );
      });
  }
  ...
  return (
    ...
    >
      <View style = {{height: Window_height*98, backgroundColor: 'white', padding: 22, alignItems: 'center', borderTopRightRadius: 10, borderTopLeftRadius: 10}}>
        <View>
          <Text style={{textAlign: 'center', fontSize: 20, fontWeight: 'bold', color: '#000000'}}>부족한 영양소</Text>
        </View>
...
export default UnderNutriModal;